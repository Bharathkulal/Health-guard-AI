"""
Heart Disease Model Training Script for HealthGuard AI.

Trains and compares Logistic Regression and Random Forest classifiers on
the UCI Heart Disease (Cleveland) dataset. Selects the best model using a
...
Usage:
    python ml/src/training/train_heart.py

Outputs:
    ml/models/heart_model.joblib          — Complete preprocessing + classifier pipeline
    ml/reports/heart_metrics.json         — Evaluation metrics (actual, not hardcoded)
    ml/reports/confusion_matrices/heart_confusion_matrix.png — Confusion matrix plot
"""
...
logger = logging.getLogger("healthguard.ml.training.heart")
...
def train_heart_model():
    """
    Complete heart disease model training pipeline:
    1. Load dataset
    2. Validate & clean
    ...
    logger.info("  Dataset: UCI Heart Disease (Cleveland)")
    logger.info("=" * 70)

    # 1. Load dataset
    data_path = os.path.join(ML_ROOT, "data", "raw", "heart_disease_cleveland.csv")
    df = load_heart_dataset(data_path)

    # 2. Clean dataset
    df, cleaning_report = clean_heart_dataset(df)
    logger.info(f"Cleaning report: {json.dumps(cleaning_report, indent=2)}")

    # 3. Split data
    X_train, X_val, X_test, y_train, y_val, y_test = split_heart_data(df, random_state=RANDOM_STATE)
    ...
    for model_name, classifier in candidate_models.items():
        ...
        pipeline = Pipeline([
            ("preprocessor", build_heart_preprocessing_pipeline()),
    ...
    cm_path = os.path.join(CONFUSION_DIR, "heart_confusion_matrix.png")
    plot_confusion_matrix(
        y_test, y_test_pred,
        title=f"Heart Disease — {best_model_name} (Test Set)",
        save_path=cm_path,
        labels=["No Disease", "Disease Present"],
    )

    # 8b. Save complete pipeline
    pipeline_path = os.path.join(MODELS_DIR, "heart_model.joblib")
    ...
    metrics_report = {
        ...
        "positive_class_prevalence": round(float(df[HEART_TARGET].mean()), 4),
        "feature_names": HEART_FEATURE_NAMES,
        "target_definition": "Presence of heart disease (binary: 0=absent, 1=present from original 0-4 scale)",
        "preprocessing_description": "Median imputation for missing values + StandardScaler normalization",
        ...
        "pipeline_file": "heart_model.joblib",
    }

    metrics_path = os.path.join(REPORTS_DIR, "heart_metrics.json")
    ...
    logger.info("\n" + "=" * 70)
    logger.info("  HEART DISEASE TRAINING SUMMARY")
    logger.info("=" * 70)
    logger.info(f"  Dataset:           UCI Heart Disease (Cleveland)")
    logger.info(f"  Records:           {len(df)}")
    logger.info(f"  Features:          {len(HEART_FEATURE_NAMES)}")
